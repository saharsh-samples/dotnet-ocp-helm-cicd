/**
 * Determines where build should be skipped based on committer
 *
 * Returns:
 *   'true' if build should run. 'false' if job was a CI commit and should be skipped
 */
boolean shouldJobRun() {

    def committer = sh(script: 'git show -s --format="%aN"', returnStdout: true).trim()
